...
export default function QuotationMoveFrom() {

    const [show, setShow] = useState(false);

    return (
        <div className='QuotationMoveMain'>
            <div className='QuotationMoveHead'>Move From (जहां से सामान जाएगा)

                <button className='btn' onClick={() => setShow((show) => !show)}>{show ? <BiCaretDown /> : <BiCaretRight />}</button></div>
            {show ? <p>{<div className='QuotationMoveForm'>
                <p className='QuotationMoveFormMain'>COUNTRY (देश)</p>
                <input type='text'></input>
                <p className='QuotationMoveFormMain'>CITY (शहर)</p>
                <input></input>
                <p className='QuotationMoveFormMain'>STATE (राज्य)</p>
                <input></input>
                <p className='QuotationMoveFormMain'>PINCODE(पिन कोड)</p>
                <input></input>
                <p className='QuotationMoveFormMain'>ADDRESS (पता)</p>
                <textarea></textarea>
